...
void loop() {
  ...
  for (int x = 0; x < WIDTH; x++) {
    for (int y = 0; y < HEIGHT; y++) {
      int red   = (noise(x * scale, y * scale,         phase * 1.0f) + 1.0) * 127;
      int green = (noise(x * scale, y * scale, 20.0f + phase * 0.6f) + 1.0) * 127;
      int blue  = (noise(x * scale, y * scale, 55.0f + phase * 0.3f) + 1.0) * 127;
